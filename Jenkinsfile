  env.DOCKERHUB_USERNAME = 'davidallentradingappscom'
  ...
  node("docker-test") {
    ...
    stage("Integration Test") {
      try {
        ...
        sh "docker run -d -P --name=cd-demo cd-demo"
  ...
  node("docker-stage") {
    ...
    stage("Staging") {
      try {
        sh "docker rm -f cd-demo || true"
        sh "docker run -d -P --name=cd-demo ${DOCKERHUB_USERNAME}/cd-demo:${BUILD_NUMBER}"
      ...
      try {
        ...
        sh '''
          ...
          if [[ "$SERVICES" -eq 0 ]]; then
            ...
            docker service create --replicas 3 --network cd-demo --name cd-demo -p 8080:58080 ${DOCKERHUB_USERNAME}/cd-demo:${BUILD_NUMBER}
          else
